...
#define W25N01GV_MAX_PAGE         65536
...
class W25N {
  ...
  public:
    ...
    /* sendData(unsigned char * buf, int len) -- Sends/recieves data to the flash chip.
     ...
     * buffer. */
    void sendData(unsigned char * buf, uint32_t len);
    ...
    int begin(uint8_t cs, uint32_t speed);
     ...
     * */
    int loadProgData(uint16_t columnAdd, unsigned char* buf, uint32_t dataLen);
    int loadProgData(uint16_t columnAdd, unsigned char* buf, uint32_t dataLen, uint32_t pageAdd);
     ...
     * WILL ERASE THE DATA IN BUF OF LENGTH DATALEN BYTES
     */
    int loadRandProgData(uint16_t columnAdd, unsigned char* buf, uint32_t dataLen);
    int loadRandProgData(uint16_t columnAdd, unsigned char* buf, uint32_t dataLen, uint32_t pageAdd);
    ...
    int read(uint16_t columnAdd, unsigned char* buf, uint32_t dataLen);
